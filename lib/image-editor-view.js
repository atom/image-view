...
export default class ImageEditorView {
  ...
  updateImageURI () {
    const src = this.editor.getEncodedURI()
    if (src.toLowerCase().endsWith('.tiff') || src.toLowerCase().endsWith('.tif')) {
      fs.readFile(src.replace('file://', ''), (err, buffer) => {
        const worker = new Worker(path.join(__dirname, 'tiff-worker.js'));
        worker.onmessage = ({data: {height, width, imageBuffer}}) => {
          ...
          this.emitter.emit('update-image')
        }
        worker.postMessage(buffer)
      })
    } else {
      this.refs.image.src = `${src}?time=${Date.now()}`
      this.emitter.emit('update-image')
    }
  }

  onDidUpdateImage (callback) {
    return this.emitter.on('update-image', callback)
  }
